  ...
  createMatch(query, user) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let matcher = await userModel.getUserByAttribute(
          "userName",
          query.matcher
        );
        let matched = await userModel.getUserByAttribute(
          "userName",
          query.matched
        );
  ...
  getUserConnectedMatchesSocketsId(userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!userMatches.length) return resolve([]);

        let result = userMatches.map(async (match) => {
          let notifiedSocket = await cacheService.getUserSocketId(
            match.matcher === userId ? match.matched : match.matcher
          );
          if (notifiedSocket) return notifiedSocket;
        });
